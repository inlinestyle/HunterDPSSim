...
function RunPlayerCombatCalculation() {
    ...
    for (let i = 31; i < 32; i++) {
        ...
        AV[i] = IF(getRange('B20').value=="No Racial",TotalFightLength*2,getRange('AV28').value);
        AW[i] = IF( getRange('G19').value==0,TotalFightLength*2,getRange('AW28').value);
        AX[i] = 0;
        AY[i] = 0;
        AZ[i] = 3;
        BB[i] = 0;
        ...
        BF[i] = IF(COUNTIF(getRange('$I$21', 'Buffs and Debuffs').value,"*Rune")==1, getRange('BF28').value,TotalFightLength);
        BG[i] = getRange('BG28').value;
        BH[i] = 0;
        BI[i] = getRange('BI28').value;
        BJ[i] = getRange('R2').value;
        BL[i] = (110+getRange('Z19').value)*MasterMarksmanAP*SurvInstinctsAP;
        ...
        BA[i] = IF(getRange('E71', 'Talents').value==1,BI[i],TotalFightLength*2);
        BP[i] = (IF(FQ[i]>0,110,0)+getRange('Z19').value)*SurvInstinctsAP;
        ...
        S[i] = IF(O[i]==20,0,Math.floor(VLOOKUP(O[i],CLReferenceTable,5,false)*IF(getRange('AD30').value>0,0.8,1),0));
        ...
        BK[i] = (IF(AA[i]>0,getRange('H20').value,0)+IF(AB[i]>0,getRange('H21').value,0)+IF(AC[i]>0,getRange('H22').value,0))*MasterMarksmanAP*SurvInstinctsAP;
        BM[i] = BK[i]+BJ[i]+BL[i];
        ...
        CH[i] = (getRange('$C$36', 'Player DPS Calc').value+BM[i]*0.15)*CG[i]*(1-BY[i]-BX[i])+(getRange('$C$36', 'Player DPS Calc').value+BM[i]*0.15)*CG[i]*BY[i]*RangeCritDamage;
        CJ[i] = BY[i]+getRange('CJ29').value*(1-getRange('R7').value);
        CK[i] = (getRange('CI28').value+BM[i]*0.2)*CI[i]*(1-CJ[i]-BX[i])+(getRange('CI28').value+BM[i]*0.2)*CI[i]*CJ[i]*RangeCritDamage;
        ...
        CV[i] = (CQ[i]+(BQ[i]+getRange('$G$23', 'Buffs and Debuffs').value)*MainhandSpeed/14)*(1-CM[i]-CR[i]-0.25)*CT[i]+ (CQ[i]+(BQ[i]+getRange('$G$23', 'Buffs and Debuffs').value)*MainhandSpeed/14)*CR[i]*MeleeCritDamage*CT[i]+ (CQ[i]+(BQ[i]+getRange('$G$23', 'Buffs and Debuffs').value)*MainhandSpeed/14)*0.25*CT[i]*0.76;
        DJ[i] = IF(P[i]==1,1,0);
        DK[i] = IF(P[i]==2,1,0);
        DL[i] = IF(P[i]==3,1,0);
        DM[i] = IF(P[i]==6,1,0);
        ...
        CD[i] = (getRange('R5').value+BM[i]*0.2)*BZ[i]*GronnStalkerSSMod*(1-CE[i]-BX[i])+(getRange('R5').value+BM[i]*0.2)*BZ[i]*GronnStalkerSSMod*CE[i]*RangeCritDamage;
    ...
    for (let i = 32; i < 33; i++) {
        ...
        DV[i] = MAX(0,IF(AT[i]>0,0,MIN(DT[i]*getRange('Z4').value+DV[i-1],1)-IF(Q[i-1]==EquippedTrinket1,1,0)));
        DW[i] = MAX(0,IF(AU[i]>0,0,MIN(DU[i]*getRange('Z5').value+DW[i-1],1)-IF(Q[i-1]==EquippedTrinket2,1,0)));
        ...
        EA[i] = IF(OR(EquippedRing1=="Band of the Eternal Champion", EquippedRing2=="Band of the Eternal Champion"), IF(BB[i]>0,0,MIN(DQ[i-1]*getRange('Z13').value+EA[i-1],1)-IF(Q[i-1]=="Band of the Eternal Champion",1,0)),0);
        EB[i] = IF(EquippedRanged!="Don Santos' Famous Hunting Rifle",0,MIN((DJ[i-1]+DK[i-1]+DL[i-1]+DM[i-1])*getRange('Z14').value+EB[i-1],1)-IF(Q[i-1]=="Santos' Blessing",1,0));
        EC[i] = MIN((DN[i-1]+DO[i-1])*getRange('Z15').value+EC[i-1],1)-IF(Q[i-1]==EnchantMainhand,1,0);
        ...
        CT[i] = CO[i];
        EF[i] = IF(AND(EquippedTrinket1=="Badge of the Swarmguard",AB[i]>0),DT[i]*getRange('Z4').value+EF[i-1],IF(AND(EquippedTrinket2=="Badge of the Swarmguard",AC[i]>0),DU[i]*getRange('Z5').value+EF[i-1],0))-IF(ET[i-1]==1,1,0);
        ...
        FT[i] = MAX(getRange('V2').value-IF(FC[i]>0,getRange('E31').value,0)-IF(FD[i]>0,getRange('E32').value,0)-IF(FE[i]>0,getRange('E33').value,0)-IF(FF[i]>0,getRange('E34').value,0),0)*(Y[i]-Y[i-1])/ActiveFightDuration;
        ...
        T[i] = IF(OR(P[i]>0,O[i]==12),IF(FM[i]==1,getRange('V12').value,0)+IF(AND(EquippedRanged=="Black Bow of the Betrayer",P[i]!=4,P[i]!=5),8,0),0) +IF(Y[i]>5*Math.ceil(Y[i-1]/5),getRange('V19').value,0) +IF(OR(P[i]==2,P[i]==3),S[i]*IF(EE[i]==1,0.4,0),0)+IF(O[i]==9,EL[i],0)+IF(EM[i]>0,400,0)+IF(O[i]==11.5,EN[i],0);
        U[i] = MIN(U[i-1]-S[i]+T[i],FinalMana);
        V[i] = MAX((getRange('V2').value-getRange('$Z$24', 'Gear').value-IF(FC[i]>0,getRange('E31').value,0)-IF(FD[i]>0,getRange('E32').value,0)-IF(FE[i]>0,getRange('E33').value,0)-IF(FF[i]>0,getRange('E34').value,0)-IF(FG[i]>0,getRange('E39').value,0))-IF(FJ[i]>0,FJ[i]*200,0)-IF(ER[i]==1,300,0)- IF(AL[i]>0, getRange('V28').value,0),0);
        W[i] = V[i]/(V[i]+400 + 85 * ((5.5 * 70) - 265.5));
        BP[i] = (IF(FQ[i]>0,110,0) + IF(AM[i]>0,getRange('H29').value,IF(FP[i]>0,getRange('H43').value,0)) + getRange('Z19').value)*SurvInstinctsAP;
        BR[i] = (IF(AL[i]>0,COUNTIF(getRange('B15').value,"*Mongoose*")*getRange('H15').value,0)+IF(AND(AB[i]>0,EquippedTrinket1=="Badge of Tenacity"),150,0)+IF(AND(AC[i]>0,EquippedTrinket2=="Badge of Tenacity"),150,0)+IF(FV[i]<=8.5,GraceofAirAgi,0))*KingsMod*(1+getRange('$E$66', 'Talents').value*0.03);
        BX[i] = MAX(0,getRange('R7').value - IF(FF[i]==1,getRange('J34').value,0));
        BY[i] = (getRange('R6').value+IF(AI[i-1]>0,getRange('AA8').value,0)+IF(FL[i-1]>0,0.03,0)+BR[i]/AgilityToCrit/100)*(1-BX[i-1]);
        CE[i] = BY[i]+getRange('CE29').value*(1-BX[i-1]);
        CJ[i] = BY[i]+getRange('CJ29').value*(1-BX[i-1]);
        CM[i] = MAX(0,getRange('$F$7', 'Player DPS Calc').value - IF(FF[i]==1,getRange('J34').value,0))+getRange('$F$8', 'Player DPS Calc').value;
        CN[i] = (getRange('R12').value+getRange('CN29').value+IF(AI[i-1]>0,getRange('AA8').value,0)+IF(FL[i-1]>0,0.03,0)+BR[i]/AgilityToCrit/100)*(1-CM[i-1]);
        CR[i] = getRange('R12').value+IF(AI[i-1]>0,getRange('AA8').value,0)+IF(FL[i-1]>0,0.03,0)+BR[i]/AgilityToCrit/100;
        DJ[i] = IF(P[i]==1,1,0);
        DK[i] = IF(P[i]==2,1,0);
        DL[i] = IF(P[i]==3,1,0);
        DM[i] = IF(P[i]==6,1,0);
        ...
        BK[i] = (IF(AA[i]>0,getRange('H20').value,0)+IF(AB[i]>0,getRange('H21').value,0)+IF(AC[i]>0,getRange('H22').value,0)+EI[i]+EQ[i]+IF(AH[i]>0,getRange('H26').value,0) +IF(AJ[i]>0,getRange('H14').value,0)+ IF(AK[i]>0,getRange('H13').value,0)+getRange('BR32').value)*MasterMarksmanAP*SurvInstinctsAP;
        BL[i] = (110+getRange('Z19').value+ES[i]+IF(AM[i]>0,getRange('H29').value+BR[i]/4,IF(FP[i]>0,getRange('H43').value,0)))*MasterMarksmanAP*SurvInstinctsAP;
        BM[i] = BK[i]+BJ[i]+BL[i];
